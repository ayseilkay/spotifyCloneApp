  ...
  return (
    <section>
        <header className='flex items-center justify-between mb-4'>
            ...
            {
                more && (
                    <NavLink className={"text-xs font-semibold uppercase hover:underline text-link tracking-wider"} to={more}> SEE ALL</NavLink>
                )
            }
        </header>
        <div className='grid grid-cols-5 gap-x-6'>
            {items.map(item=>(
                <NavLink key={item.id} to={"/"} className="bg-footer p-4 rounded hover:bg-active group">
                   ...
                   <button className='w-10 h-10 rounded-full shadow-2xl bg-primary absolute group-hover:flex group-focus:flex bottom-2 right-2  items-center justify-center hidden hover:scale-[1.09] '>
